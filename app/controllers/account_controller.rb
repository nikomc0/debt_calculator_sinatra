...
class AccountsController < ApplicationController
  ...
  post '/accounts' do
    ...
    if user.accounts.exists?(account_name: @account[:account_name])
      ...
      user.update_global_variables
      user.update_payment_schedule(current_user, @account)
  ...
  get '/accounts/:id/min_payment' do 
    @account = Account.find(params[:id])

    @account.get_minimum_payment(params)
  end
  ...
  patch '/accounts/:id' do
    ...
    if @account.update(changes)
      flash[:success] = "Account was saved."
      current_user.update_global_variables
      current_user.update_payment_schedule(current_user, @account)
